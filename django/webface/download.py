...
from .models import DataSettings, SecurityMeta, Financials, BalanceSheet, Dividends, SecurityPrice, SecurityList
...
class DownloadCompanyData:
    def __init__(self, tickers, updates=True):
        ...
        # Parameters
        self.id_table = self.get_id_table(tickers)
        ...
        if updates:
            ...
            self.the_tickers = self.get_missing(tickers)
    ...
    def get_missing(self, proposed_tickers):
        # For the proposed tickers, get the matching security_id
        proposed_ids = list(self.id_table[self.id_table.symbol.isin(proposed_tickers)].security_id)

        # Check if any ids are missing from any database
        current_ids = []
        for db in self.DB_ref.keys():
            current_ids += list(self.DB_ref[db].objects.all().values_list('security_id', flat=True))
        missing_ids = set(proposed_ids).difference(current_ids)

        # Get the symbol for any missing ids
        missing_sym = list(self.id_table[self.id_table.security_id.isin(missing_ids)].symbol)
    ...
    def get_data(self):
        ...
        for t in pbar(stock_data.symbols):
            ...
            try:
                ...
                info, financials, balancesheet = attrgetter('info', 'financials', 'balancesheet')(obj)
...
class DownloadData_old:
    def __init__(self, tickers, get_prices=False, updates=True):
        # Initialize dataframes
        self.meta = pd.DataFrame()
        self.financials = pd.DataFrame()
        self.balancesheet = pd.DataFrame()
    ...
    def get_missing(self, proposed_tickers):
        ...
        # Check if any ids are missing from any database
        current_ids = []
        for db in set(self.DB_ref).difference(['prices']):
            current_ids += list(self.DB_ref[db].objects.all().values_list('security_id', flat=True))
        missing_ids = set(proposed_ids).difference(current_ids)
    ...
    def get_data(self):
        ...
        if self.get_prices:
            ...
            # Convert to multilevel if single ticker
            if len(self.the_tickers) == 1:
                midx = tuple((self.the_tickers[0], x) for x in prices.columns)
                prices.columns = pd.MultiIndex.from_tuples(midx)

            #Flatten out so that symbol is column
            self.prices = prices.stack(0).reset_index().rename(columns={'level_1': 'symbol'})
